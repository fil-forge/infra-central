...
#
# The hostname suffix comes from the stage's Pulumi config, so the hostnames are
# the ones the ALB and the certificate were built from.
#
# Prerequisites:
#   - curl and jq
...
STACK_CONF="$ROOT/pulumi/platform/Pulumi.$STAGE.yaml"

[ -f "$STACK_CONF" ] || {
  echo "ERROR: no such stage '$STAGE': $STACK_CONF does not exist" >&2
  exit 1
}

# Read from the committed config rather than `pulumi stack output`, so the check
# needs no Pulumi login, no state access and no AWS credentials. The suffix is
# what the stage was built from either way.
SUFFIX="$(sed -n 's/^[[:space:]]*forge-central:hostnameSuffix:[[:space:]]*\(.*\)[[:space:]]*$/\1/p' "$STACK_CONF" | tr -d '"'\''')"
[ -n "$SUFFIX" ] || {
  echo "ERROR: no forge-central:hostnameSuffix in $STACK_CONF" >&2
  exit 1
}
