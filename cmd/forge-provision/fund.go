...
//
// Unlike seed and vault, Terraform never calls this. An apply must not move
// money, so the caller is an operator running `make fund-payer`, which shows the
// plan and asks before it lets this sign anything.
//
