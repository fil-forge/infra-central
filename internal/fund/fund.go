...
//
// Nothing here is called by Terraform. An apply must never move money, so the
// caller is an operator running `make fund-payer`, and Plan runs on its own to
// show what would happen before anything is signed.
package fund
