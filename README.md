...
Infrastructure is [Pulumi](https://www.pulumi.com) in Go, so the deployment and
the Lambda it deploys are one Go module with one formatter, one vet and one test
command.
...
| Decision                                        | Why                                                                                                                                                                                                                             |
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Secrets minted by a Lambda in the VPC**       | Private keys are generated where they will be used. Nothing is written to a laptop, and no key enters stack state: the function returns only DIDs, addresses and names.                                                          |
...
| **One program per project, one stack per stage** | A stage's config file says what differs and nothing else. The wiring is stated once, in Go, so two stages cannot drift into different shapes.                                                                                    |
| **Two stacks per stage**                        | `platform` holds the VPC, RDS, OpenBao and ingress; `apps` holds the services. A routine image bump previews in seconds and never touches the database.                                                                          |
...
- **hilt and swarf accept the identity key only as a file path**, and the
  ...
  injects secrets as environment variables, so the `ecsservice` package wraps
...
- **Migrations run in-process** via goose for sprue, hilt, swarf and plc.
  Concurrent starts race on the goose lock, so services run at a desired count
  of 1 until someone sets the relevant `*_SKIP_MIGRATIONS`.
...
- **Every preview warns that `failureThreshold` is deprecated.** Expected, and
  the alternatives are worse: AWS fixed the Cloud Map custom health check wait at
  ...
  which every preview schedules a replacement that lands in the same state. The
  comment in `pulumi/internal/ecsservice/ecsservice.go` has the full story and
  tracks the upstream issues that would end the warning.
- **Anything rendered into JSON is sorted deliberately.** Go map iteration order
  is random, so an unsorted environment block or entrypoint prelude would rewrite
  the task definition on every run and restart every task. `wrapper_test.go`
  holds that line.
...
pulumi/
  ...
  platform/              VPC, RDS, S3, DynamoDB, ALB, OpenBao, provision Lambda
  apps/                  the six ECS services
  internal/              the components; see Stages below
  README.md              how the tree is organised, and the Terraform it replaced
```

Each project directory holds a `Pulumi.yaml`, a `main.go` and one
`Pulumi.<stack>.yaml` per stage. `pulumi/README.md` is the orientation document
for the infrastructure code itself; this file is the runbook.

## Stages

A stage is a **stack name**, shared by the `platform` and `apps` projects.
Everything is namespaced by it, so stages coexist in one AWS account without
colliding:
...
The stack name *is* the stage: the programs read it from `ctx.Stack()` and
nothing else names it, so a stage's label and the resources it refers to cannot
come apart.
...
Six stacks across three projects:

| Project                     | Stacks                                 |
| --------------------------- | -------------------------------------- |
| `forge-central-bootstrap`   | `nonprod-us-east-2`, `prod-us-east-2`  |
| `forge-central-platform`    | `dev`, `prod`                          |
| `forge-central-apps`        | `dev`, `prod`                          |

Files:

```
pulumi/platform/
  main.go                  reads config, builds the composite, exports outputs
  Pulumi.dev.yaml          committed, non-secret: DNS, chain, contracts, digest
  Pulumi.prod.yaml         the same keys, prod's values

pulumi/apps/
  main.go                  reads the platform stack, builds the six services
  Pulumi.dev.yaml          committed: image digests
  Pulumi.prod.yaml         digests commented out; prod is not deployed
```

Both programs stay short because `internal/platform` and `internal/apps` hold
the wiring. That is the point of the split: a stack's config says what differs,
and nothing else can drift between stages.

The component tree mirrors that split:

```
pulumi/internal/
  platform/                everything the platform stack builds
    platform.go            the wiring, calling the seven below
...
A component used by exactly one project lives under that project's composite.
`ecsservice` is the one that genuinely crosses the boundary, used by `apps` and
by `openbao` inside `platform`.

Chain configuration lives in the **platform** stack and the apps stack reads it
from there, so a stage has one set of contract addresses rather than two copies
to keep in step. That mirrors smelt's shared `smart-contracts.env`.

`apps` reads `platform` through a **stack reference**, which is also what orders
the two: the reference makes the platform stack's outputs a dependency, so an
apps run cannot proceed against outputs that do not exist yet.
...
- **`forge-central:zoneName`** is the delegated Route53 zone records are written
  into. Every non-prod stage shares `forge-sandbox.fil.one`.
- **`forge-central:hostnameSuffix`** is what that stage's hostnames end with,
  which for non-prod includes the stage label.

The delegation itself lives in
[fil-one/infrastructure](https://github.com/fil-one/infrastructure) and is added
once per account: a Route53 zone for the delegated name, plus a Cloudflare `NS`
record carrying that zone's four name servers, named `forge-sandbox` in the
non-prod account and `forge` in the prod one.
...
The `ingress` component issues `*.<hostnameSuffix>`, writes the DNS validation
record into the delegated zone, and waits for validation. Two reasons it
...
**`pulumi destroy` deletes no parameter this project generates.** The provision
Lambda creates them, so no stack has a record of them and none is ever removed.
An accidental destroy therefore cannot burn a funded wallet or invalidate a DID
that storage providers have already registered against.
...
fail the next run that tried to rebuild it. The stage's key seals OpenBao and
...
- **Pulumi CLI**, logged in to the `Filecoin_Foundation` organization
  (`pulumi login`). State and any secret output live in Pulumi Cloud; nothing
  is kept on a laptop.
...
| ----------------------- | ----------------------------------------------- |
| `bootstrap` stacks      | `pulumi up` run locally, always                 |
| provision image         | `make publish` run locally, pushed to ECR       |
| dev `platform`, `apps`  | `pulumi up` run by an operator                  |

**Every deploy is currently an operator running `pulumi up`.** There is no
automatic deployment on merge; reinstating one is [planned
work](#deploy-on-merge-again). What is committed is still what should be
deployed — every per-stage value lives in a `Pulumi.<stack>.yaml` in this
repository — but nothing enforces that today, so a merged digest reaches a stage
only when someone runs the stack.

Ordering matters: `platform` before `apps`, because `apps` reads its outputs
through a stack reference.

AWS credentials are not stored in the stack. An operator runs with credentials
for the target account, and each stack names the account it belongs to, so
credentials for the wrong one fail the preview rather than building a second
working copy of the stage somewhere unexpected.

Prod is not deployed yet: `Pulumi.prod.yaml` is committed for both projects, its
contract addresses are deliberately `REPLACE_ME`, and its image digests are
commented out.
...
The bootstrap stack is always run locally. It runs rarely, once per account and
region, and it creates the registry that everything else depends on, so there is
nothing for a pipeline to trigger on.

```bash
pulumi -C pulumi/bootstrap stack select nonprod-us-east-2
pulumi -C pulumi/bootstrap up
```

This creates `forge-central/provision`, the ECR repository for the provision
Lambda image. There is one bootstrap stack per account and region: ECR
repositories are regional, Lambda pulls an image only from ECR in the same
region as the function, and a pull from another account needs a repository
policy this project does not create. Stages sharing an account and region share
...
Add a stack and its config file. There is no directory to copy:

```bash
pulumi -C pulumi/bootstrap stack init nonprod-us-west-2
pulumi -C pulumi/bootstrap config set --stack nonprod-us-west-2 aws:region us-west-2
pulumi -C pulumi/bootstrap config set --stack nonprod-us-west-2 forge-central:account nonprod
pulumi -C pulumi/bootstrap up --stack nonprod-us-west-2
```
...
Two stacks and two config files, and no code changes at all:

```bash
pulumi -C pulumi/platform stack init staging
pulumi -C pulumi/apps     stack init staging

cp pulumi/platform/Pulumi.dev.yaml pulumi/platform/Pulumi.staging.yaml
cp pulumi/apps/Pulumi.dev.yaml     pulumi/apps/Pulumi.staging.yaml
```

Then, in the copies:

1. In `platform/Pulumi.staging.yaml`, set `forge-central:hostnameSuffix` to
   `staging.forge-sandbox.fil.one` and leave `forge-central:zoneName` alone: the
   zone is already delegated and shared by every non-prod stage, so the DNS
   project needs no change. Point the `forge-central:chain` block at the network
   this stage transacts against.
2. Set `forge-central:provisionImageDigest` by running
   `make publish STAGE=staging`, which writes it for you.
3. In `apps/Pulumi.staging.yaml`, set the six `forge-central:imageDigests`.

Nothing needs to be told the stage's name, the `Stage` tag, or which resources to
prefix — all three come from the stack name. Nothing needs trigger patterns
either, because there are no per-directory triggers to keep in step.

Prod differs from dev in its config rather than by being a different shape:
multi-AZ database, deletion protection on, a larger OpenBao connection budget,
and a digest copied from dev when a change is promoted rather than written by
...
`platform` brings up the VPC, RDS, OpenBao and the secrets; `apps` then brings up
the six services.

The first `platform` run is slow: it waits for the OpenBao task's cold start
before it can initialise it, inside a synchronous Lambda call that Lambda caps at
15 minutes. If it times out there, run it again — the seed phase regenerates
nothing that already exists, which is what protects funded wallets.

### A personal sandbox stage

Stage names are not limited to dev and prod. `pulumi stack init sandbox-<you>`
plus a copied config file is the whole setup, and it is the fastest loop for
iterating on the provision Lambda: no commit, no merge, nothing to wait for.
What it costs is review — nothing about a stack run from a laptop is visible to
anyone else. Use it to iterate, not to host anything anyone depends on.
...
```bash
pulumi -C pulumi/platform stack output walletAddresses --stack dev
```
...
No deployment invokes this phase. Bringing up a stage must not move money, so
funding is always an explicit operator action.
...
`make publish` writes the new digest into the stage's `Pulumi.<stage>.yaml`, so
there is no line to edit by hand. **Commit that file**, so what is deployed stays
visible in git rather than only in a stack.

Promoting the same image to prod is a copy of that digest into
`pulumi/platform/Pulumi.prod.yaml`, done deliberately when the change is ready
rather than as a side effect of a build.

### Deploying a service

Change its digest in the stage's `forge-central:imageDigests`, commit, and run
`pulumi -C pulumi/apps up`. Every stage pins digests, dev included: a rolling
tag would make what dev runs depend on when a task last restarted rather than on
what was merged.

### Confirming nothing was regenerated

The most important check after any run:

```bash
pulumi -C pulumi/platform stack output createdParameters --stack dev
```

Empty means every key already existed and was reused. A non-empty list after
the first run of a stage means something was minted; find out what before
...
The script reads `forge-central:hostnameSuffix` from the stage's
`Pulumi.<stage>.yaml`, so it needs no Pulumi login and no state access.
...
Delete the parameter, then bump the seed trigger and run the stack:

```bash
aws ssm delete-parameter --name /forge-central/dev/swarf/identity

pulumi -C pulumi/platform config set --stack dev forge-central:seedTrigger 3
pulumi -C pulumi/platform up --stack dev
```

The bump is required. The phase is a Lambda invocation resource, which re-invokes
only when its input changes, and deleting a parameter changes nothing the stack
can see. The trigger is part of the input, so changing it is what forces the
call — and because it is stack config, the change is a committed diff rather than
an invisible action.

The new DID appears in `serviceDids`, and the rotation also refreshes
...
rather than something a routine run does on its own.

Rotating an identity that _signs_ a proof — sprue, indexer or etracker —
re-issues that proof automatically in the same run, because the old delegation
would no longer verify against a key that does not exist.

#### Why proofs are not rewritten every run
...
different bytes and a different CID. Rewriting on every run would churn the
...
which is what `SecretFilesBase64` on `internal/ecsservice` is for. hilt's proof
is a `base64+gzip` container, already text, and travels as it is.
...
pulumi -C pulumi/platform config set --stack dev forge-central:vaultTrigger 2
pulumi -C pulumi/platform up --stack dev
```
...
One Go module covers the Lambda and the infrastructure, so there is one of each
command rather than two. Both run offline against no deployed stage — the Pulumi
components build against a mocked resource monitor and need no credentials,
backend or network — which is why `make smoke` is separate: it needs a stage to
be up. See [Smoke-testing a stage](#smoke-testing-a-stage).

`pulumi preview` is the other check worth running before a change lands, and it
does need credentials for the target account.
...
Configure GitHub Branch Protection rules to require passing CI checks and a
`pulumi preview` on the affected stacks before a PR can be merged.

### Deploy on merge again

Deploys are operator-run `pulumi up` today. Before the migration from Terraform,
the dev stage deployed itself: both of its workspaces tracked `main` with
auto-apply on, so a merge reached dev without anyone running anything, and
applies executed on a runner rather than on a laptop.

That property is worth having back, and there are two ways to it: Pulumi
Deployments with a push trigger on `main`, or a GitHub Actions workflow running
`pulumi up` against an OIDC-federated role. Either one needs the same two things
settled — that `platform` runs before `apps`, and that prod stays behind an
explicit confirmation.

Until then, a merged digest is not a deployed digest, which is the one respect in
which this is a step back from the HCP setup.

### Automated post-deploy checks

After a stack is brought up, run the smoke tests to verify that the stage is up
and running correctly.

### Onboarding a regional appliance has no tooling

Everything the central services need is minted and wired by bringing up a stage.
The first Piri/Ingot appliance pointed at a stage needs five more things, and this
...
These are runtime state, not configuration, so no deployment creates them. smelt
does each one from the operator's machine against the box; the equivalents here
have no home yet.
...
Fargate tasks in private subnets and ECS Exec is not enabled on any service, so
`aws ecs execute-command` does not work either. Three ways out:
...
`make publish` is still run by hand from a developer machine with credentials
for the target account, and the digest it writes has to be committed.

A workflow on merge to `main` that publishes the image and commits the digest
would close that step. Ordering needs care: the workflow has to publish before it
writes, and write only what it published. It also pairs with [deploying on merge
again](#deploy-on-merge-again) — publishing a digest nothing deploys is only half
the loop.

### Deploy service changes from their own repositories

Today a service is deployed by editing `forge-central:imageDigests` here. It
should instead happen when a commit lands on the service's own `main`.
...
which commits that digest to the dev stage. The commit is what should deploy, so
dev stays visible in git rather than in a stack nobody reads.
...
`pulumi/internal/platform/openbao` already renders at task start. Two things
need checking before it goes in. A device that cannot write makes OpenBao reject
...
final snapshot on delete unless a stage turns protection off. Nothing says how to
use them. A restore is also more than the services' data, because OpenBao's
storage lives in the same instance: rolling the database back rolls back hilt's
AppRole and the root of trust regional appliances unseal against. Write the
procedure, and decide as part of it whether OpenBao's storage should move to an
instance of its own.
...
- [pulumi/README.md](pulumi/README.md) — how the infrastructure code is
  organised, and what each piece of the Terraform it replaced became.
